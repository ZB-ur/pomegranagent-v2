...
function schedulePage(){
  ...
  <div class="calendar-layout"><div><section class="panel calendar-panel" aria-label="月历"><div class="weekdays">${['一','二','三','四','五','六','日'].map(d=>`<span>周${d}</span>`).join('')}</div><div class="calendar-grid" style="--weeks:${cells/7}" role="group" aria-label="选择排班日期">${calendar}</div><div class="calendar-foot"><span class="legend"><i class="dot"></i>已安排</span><span class="legend"><i class="dot green"></i>✓ 已记录</span><span>方向键选日期 · 回车编辑</span></div></section><p class="calendar-summary">本月已安排 ${scheduled} 天 · 周末也可以安排活动。<br>当天名单会直接出现在儿童入口。</p></div><section class="panel day-editor" aria-label="当天安排">${dayEditor()}</section></div>`;
}
function dayEditor(){
